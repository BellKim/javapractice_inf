...
select * from board;
          ...
          num    ref     re_step(출력순서) re_level(출력단계)
제목     1        1           0             0
제목     2        2           0             0
답변     3        1           2             1
답변     4        1           1             1
...
order by ref desc,re_step asc)a)b
 ...
 select b.*
from (select rownum as rm, a.* from (select * from board order by ref desc,re_step asc)a)b
 where b.rm>=1 and b.rm<=5;
...
(select rownum as rm, a.* from(
  ...
  order by ref desc, re_step asc) a)b
where b.rm >=1  and b.rm<=5;	--where 절 : page 번호 순서. 
